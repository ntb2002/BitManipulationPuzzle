 ...
 */
int fourthBits(void) {
  int x = 0x88; // evaluates to 10001000
  return (x << 24) | (x << 16) | (x << 8) | x; // shifts x over and compares so 1 appears every 4 bits
}
/* 
...
int isNotEqual(int x, int y) {
  int a = (x^y);
  return !!a;  // converts non zeros to 1
}
